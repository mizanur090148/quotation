...
class ScheduleController extends BaseController
{
    ...
    *
    ...
    {       
        try {   
            $request->request->add(['from_date' => $request->range['start']]);
            $request->request->add(['to_date' => $request->range['end']]);  
          
            $modelData = $this->apiCrudHandler->store(array_merge($request->all(), ['from_date' => '2021-01-22', 'to_date' => '2021-01-22']), Schedule::class);           
            return $this->sendResponse($modelData);
        } catch (Exception $ex) {
            return $this->sendError($e->getMessage());
        }
