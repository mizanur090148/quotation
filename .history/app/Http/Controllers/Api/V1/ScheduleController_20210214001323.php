    ...
    *
    ...
    public function getVisitScheduleData(Request $request)
    {
        try {
            ...
            $results = [];
            foreach ($modelData as $key => $data) {dd(Carbon::createFromFormat('Y-m-d', $data->to_date)->addDay()->format('Y-m-d'));
                $results[$key]['title'] = $data->from_location . ' - ' .  $data->to_location;
                $results[$key]['start'] = $data->from_date;
                $results[$key]['end'] = Carbon::createFromFormat('Y-m-d', $data->to_date)->addDay();
            }
