    ...
    *
    ...
    public function getVisitScheduleData()
    {
        try {
            ...
            $results = [];
            foreach ($modelData as $key => $result) {
                $results[$key]->title = $result->from_location . ' - ' .  $result->to_location;
                $results[$key]->start = $result->form_date;
                $results[$key]->end = $result->to_date;
            }
