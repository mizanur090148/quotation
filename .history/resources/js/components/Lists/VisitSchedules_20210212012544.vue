            ...
            <div class="table-responsive">
              <table class="list-table table-hover">
                ...
                <tbody v-if="schedules.length > 0">
                  <tr v-for="(schedule, index) in schedules" :key="schedule.id" :class="(schedule.status == 1) ? 'bg-gradient-success' : (schedule.status == 2) ? 'bg-gradient-warning' : (schedule.status == 3) ? 'bg-gradient-danger' : ''">
                      <td>{{ ++index }}</td>
