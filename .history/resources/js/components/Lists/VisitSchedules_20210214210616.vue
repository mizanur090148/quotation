            ...
            <div class="table-responsive">
              <table class="list-table table-hover">
                ...
                <tbody v-if="schedules.length > 0">
                  <tr v-for="(schedule, index) in schedules" :key="schedule.id" :class="(schedule.status == 0) ? 'text-primary' : (schedule.status == 1) ? 'text-success' : (schedule.status == 2) ? 'text-warning' : (schedule.status == 3) ? 'text-danger' : ''">
                      <td>{{ ++index }}</td>
                      ...
                      <td>{{ schedule.note }}</td>
                      <td>{{ schedule.admin_comment }} {{ user.role + 'ASA' }}</td>
