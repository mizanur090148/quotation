          ...
          <div class="card-body">
            ...
            <form class="forms-sample" @submit.prevent="">
                  ...
                  <div class="form-group">
                    ...
                    <div class="input-group col-xs-12">
                      <select v-model="form.invoice_id" @change="getInvoiceWiseDetails" class="form-control form-control-sm" :class="{ 'is-invalid': errors.invoice_no }">
                        ...
                        <option v-for="(invoice, key) in invoices" :value="invoice.id" :key="key">{{ invoice.invoice_number }}</option>
                      </select>  
                    </div>
                   ...
                   <thead>
                    ...
                    <tbody>
                      <tr v-for="(sale_product, index) in sale_detail_list" :key="index">
                        ...
                        <td class="text-center" v-if="sale_detail_list.length > 1">
              ...
              <div class="text-right">
                <button type="submit" v-if="sale_detail_list.length" class="btn btn-sm btn-primary"                    
                    @click="returnProduct('invoice', sale_invoice.id)">Return Invoice</button>               
              </div>
          </div>
        </div>
      </div>      
...
<script>  
  ...
  export default {
    ...
    methods: {
      getSaleInvoicesDropdown() {
        axios.get('/get-sale-invoices-dropdown')
          .then((res) => {
            ...
            buttons: [
              {
                text: "Yes",
                action: toast => {
                  axios.get('sale-return/', { params: { delete_id: delete_id }})
                    .then((res) => {
                      if (res.status) {
                        if (type == 'invoice') {
                          this.sale_detail_list = [];
